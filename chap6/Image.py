...
class Image:
    ...
    if USE_GETATTR:
        def __getattr__(self, name):
            """
            __getattr__ is a special method that gets called when Python 
            can't find an attribute on an object.
            An example:
            class MyClass:
                def __getattr__(self, name):
            ...
            obj = MyClass()
            print(obj.some_attribute)  # Calls __getattr__('some_attribute')
            # Output: Looking for attribute: some_attribute
            #         Default value for some_attribute
